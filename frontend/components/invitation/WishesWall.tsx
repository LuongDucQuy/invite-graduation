...
export function WishesWall({ token, initialMessages, defaultGuestName, graduateName }: WishesWallProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newMessage = await InvitationService.postMessage(token, content.trim(), customName.trim() || defaultGuestName);
      setMessages((prev) => [newMessage, ...prev]);
      setContent('');
      success('Đã gửi lời chúc!', `Lời chúc của bạn đã được gửi đến ${graduateName}! 🎉`);
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 mb-1.5 flex items-center gap-1.5">
                <User className="w-3.5 h-3.5 text-champagne-600" /> Tên người gửi
              </label>
              <input
                type="text"
                value={customName}
            ...
            {/* Quick preset pills */}
            <div>
              <p className="text-xs text-slate-400 mb-2">Gợi ý lời chúc nhanh:</p>
        ...
        <div className="space-y-4">
          <h4 className="font-serif text-xl font-bold text-slate-800 flex items-center gap-2">
            ...
            Lời chúc từ mọi người ({messages.length})
          </h4>

          <AnimatePresence>
            {messages.length === 0 ? (
              <p className="text-center py-8 text-slate-400 text-sm font-light">
                Chưa có lời chúc nào. Hãy là người đầu tiên gửi lời chúc đến {graduateName}!
              </p>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full bg-champagne-100 text-champagne-800 font-serif font-bold text-sm flex items-center justify-center">
                        {msg.guestName ? msg.guestName.charAt(0).toUpperCase() : 'G'}
                      </div>
