...
import { Calendar, Clock, MapPin, Sparkles, Navigation, Shirt } from 'lucide-react';
...
export function EventDetails({ event }: EventDetailsProps) {
  ...
  const handleOpenMap = () => {
    if (event.googleMapUrl) {
      ...
      const query = encodeURIComponent(`${event.venueName}, ${event.venueAddress}`);
    ...
    <section className="relative py-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <h3 className="font-serif text-3xl md:text-4xl text-slate-900 font-bold tracking-tight">
            Thời Gian &amp; Địa Điểm
          </h3>
          <p className="text-slate-500 text-sm mt-2 max-w-lg mx-auto font-light">
            Trân trọng kính mời quý khách đến chung vui cùng Tân Cử nhân theo thông tin chi tiết dưới đây
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-xs uppercase tracking-widest text-champagne-600 font-semibold mb-1">
              NGÀY TỔ CHỨC
            </p>
            <h4 className="font-serif text-xl font-bold text-slate-800 capitalize mb-1">
              {formattedDateString}
            </h4>
          ...
          >
            ...
            <p className="text-xs text-slate-500">
              {event.endTime ? `Kết thúc dự kiến: ${event.endTime}` : 'Vui lòng đến đúng giờ'}
            </p>
          </motion.div>
          ...
          >
            ...
            <h4 className="font-serif text-lg font-bold text-slate-800 mb-1 leading-snug">
              {event.venueName}
            </h4>
            <p className="text-xs text-slate-500 line-clamp-2">{event.venueAddress}</p>
          </motion.div>
        </div>
        ...
        <motion.div
          ...
          className="mt-8 p-6 rounded-2xl bg-gradient-to-r from-cream-100 via-white to-cream-100 border border-champagne-200 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm"
        >
          <div className="flex items-center gap-4 text-center sm:text-left">
            <div className="p-3 rounded-full bg-champagne-100/80 text-champagne-700">
              <Shirt className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider text-champagne-700 font-semibold">
                DRESS CODE GỢI Ý
              </p>
              <p className="text-sm font-medium text-slate-800">
                {event.dressCode || 'Trang phục lịch sự / Semi-formal (Trắng, Be, Kem, Pastel)'}
              </p>
            </div>
          </div>
